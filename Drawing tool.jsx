...
import drawMonkeys from './helper/DrawMonkeys';
import { UserContext } from '../../../context/UserContext';
import loadMonkeyImages from './helper/LoadMonkeyImages';
...
function Atlas() {
	const { ownedMonke } = useContext(UserContext);

	const [monkeyClass, setMonkeyClass] = useState([]);
	...
	useEffect(() => {
		loadMonkeyImages(ownedMonke, setMonkeyClass);
	}, [ownedMonke]);

	useEffect(() => {
		if (mapReady && monkeyClass.length > 0) {
			drawMonkeys(canvasRef.current, monkeyClass, mapRef.current);
		}
	}, [monkeyClass, mapReady]);
	...
	return (
		<div className="atlas__container" style={{ zIndex: 3000 }}>
			<canvas
				...
				ref={canvasRef}
				onMouseMove={(e) => {
					onMMove(e, canvasRef.current, mapRef.current, monkeyClass, domains, regionI.current);
				}}
				onClick={(e) => {
					onMClick(
						e,
